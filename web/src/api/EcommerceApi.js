const ECOMMERCE_API_URL = import.meta.env.VITE_API_URL || "http://localhost:8081";
const ECOMMERCE_STORE_ID = "store_local";
const USE_LOCAL_API = !import.meta.env.VITE_ECOMMERCE_API_URL;

const REMOTE_API_URL = import.meta.env.VITE_ECOMMERCE_API_URL || "https://api-ecommerce.hostinger.com";
const REMOTE_STORE_ID = import.meta.env.VITE_ECOMMERCE_STORE_ID || "store_01KX7SK9AYP2597W6ETEZTQ35E";

function apiBase() {
  return USE_LOCAL_API ? ECOMMERCE_API_URL : REMOTE_API_URL;
}

function storePrefix() {
  return USE_LOCAL_API ? "/api/store" : `/store/${REMOTE_STORE_ID}`;
}

function normalizeLocalVariant(v) {
  return {
    id: String(v.id),
    title: v.title || "",
    image_url: v.image_url || v.images?.[0]?.url || null,
    sku: v.sku || null,
    price_in_cents: v.price_in_cents ?? v.price_cents ?? 0,
    sale_price_in_cents: v.sale_price_in_cents ?? v.sale_price_cents ?? null,
    currency: v.currency || "usd",
    currency_info: { code: (v.currency || "usd").toUpperCase(), symbol: "$", decimal_digits: 2 },
    price_formatted: v.price_formatted || formatCurrency(v.price_in_cents ?? v.price_cents ?? 0, { symbol: "$", decimal_digits: 2 }),
    sale_price_formatted: v.sale_price_formatted || null,
    manage_inventory: v.manage_inventory !== false,
    weight: v.weight ?? null,
    options: [],
    inventory_quantity: v.inventory_quantity ?? null,
    images: (v.images || []).map((img, order) => ({
      url: img.url || img,
      order: img.sort_order ?? order,
      type: "variant",
    })),
  };
}
...
function normalizeLocalProduct(product) {
  ...
  const price_in_cents = product.price_in_cents ?? variants[0]?.price_in_cents ?? 0;
  ...
  return {
    ...
    order: product.sort_order ?? product.order ?? 0,
    site_product_selection: "lowest_price_first",
    images: (product.images || []).map((img, order) => ({
      url: img.url || img,
      order: img.sort_order ?? order,
      type: "image",
    })),
    options: [],
    variants,
    collections: product.category_id
      ? [{ product_id: String(product.id), collection_id: String(product.category_id), order: 0 }]
      : [],
    additional_info: [],
    type: { value: product.category?.type || "" },
    custom_fields: [],
    related_products: [],
...
export async function getProducts({
  ...
  if (subcategory) queryParams.append("subcategory", String(subcategory));
  if (page && !offset) {
    const p = Number(page) || 1;
    const per = Number(perPage || limit || 24);
    queryParams.set("limit", String(per));
    queryParams.set("offset", String((p - 1) * per));
  }

  const queryString = queryParams.toString();
  const url = `${apiBase()}${storePrefix()}/products${queryString ? `?${queryString}` : ""}`;
  ...
  if (USE_LOCAL_API) {
    return {
      ...
      offset: data.offset,
      limit: data.limit,
      products: (data.products || []).map(normalizeLocalProduct),
    };
  }
...
export async function getProduct(id, { field } = {}) {
  ...
  const queryString = queryParams.toString();
  const url = `${apiBase()}${storePrefix()}/products/${id}${queryString ? `?${queryString}` : ""}`;
  ...
  if (USE_LOCAL_API) {
    return normalizeLocalProduct(data.product || data);
  }
...
export async function getProductQuantities({ fields, product_ids }) {
  ...
  const url = `${apiBase()}${storePrefix()}/variants?${queryParams.toString()}`;
...
export async function getCategories() {
  const url = `${apiBase()}${storePrefix()}/categories${USE_LOCAL_API ? "?flat=1" : ""}`;
  ...
  if (USE_LOCAL_API) {
    return {
      categories: (data.categories || []).map((collection) => ({
        id: String(collection.id),
        title: collection.name || collection.title,
        description: collection.description || null,
        image_url: collection.image_url,
        store_id: "store_local",
        ...
        metadata: {
          ...
          description: collection.description || null,
          fabrics: Array.isArray(collection.fabrics) ? collection.fabrics : [],
          prints: Array.isArray(collection.prints) ? collection.prints : [],
        },
        fabrics: Array.isArray(collection.fabrics) ? collection.fabrics : [],
        prints: Array.isArray(collection.prints) ? collection.prints : [],
      })),
...
/**
 * GET /api/store/branding — storefront theme, fonts, hero, banners
 */
export async function getStoreBranding() {
  if (!USE_LOCAL_API) {
    return null;
  }
  const response = await fetch(`${apiBase()}${storePrefix()}/branding`, {
    method: "GET",
...
async function getCheckoutLanguage() {
  if (USE_LOCAL_API) return "en";
  const response = await fetch(
    `${apiBase()}${storePrefix()}/settings`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
export async function initializeCheckout({
  ...
  const url = `${apiBase()}${storePrefix()}/checkout`;

  if (USE_LOCAL_API) {
    throw new Error("Checkout is not configured for the local Go API yet. Manage catalog via /admin.");
  }
